...
import com.myebook.homepage.member.db.MemberDAO;
...
public class MemberServiceImpl implements MemberService {
	...
	public void check_id(String user_id, HttpServletResponse response) throws Exception {
		...
		out.println(manager.check_id(user_id));
		out.close();
	}
	...
	public int join_member(MemberDTO member, HttpServletResponse response) throws Exception {
		...
		if (manager.check_id(member.getUser_id()) == 1) {
			out.println("<script>");
	...
	public void send_mail(MemberDTO member, String div) throws Exception {
		...
		String hostSMTPid = "couple234@naver.com";
		String hostSMTPpwd = "whisper1223";

		// 보내는 사람 EMail, 제목, 내용
		String fromEmail = "couple234@naver.com";
		String fromName = "Spring Homepage";
		String subject = "";
		String msg = "Mebook에 가입해주셔서 감사합니다";
		...
		if (div.equals("join")) {
			...
			msg += member.getUser_id() + "님 회원가입을 환영합니다.</h3>";
			msg += "<div style='font-size: 130%'>";
			msg += "하단의 인증 버튼 클릭 시 정상적으로 회원가입이 완료됩니다.</div><br/>";
			msg += "<form method='post' action='http://localhost:8080/myebook/approval_member.do'>";
	...
	public void approval_member(MemberDTO member, HttpServletResponse response) throws Exception {
		...
		if (manager.approval_member(member) == 0) { // 이메일 인증에 실패하였을 경우
			...
			out.println("location.href = './move_index.do';");
			out.println("</script>");
			out.close();
		}
	}
	...
	public MemberDTO login(MemberDTO member, HttpServletResponse response) throws Exception {
		...
		PrintWriter out = response.getWriter();
		// 등록된 아이디가 없으면
		if (manager.check_id(member.getUser_id()) == 0) {
			out.println("<script>");
			...
			member = manager.login(member.getUser_id());
			// 비밀번호가 다를 경우
			if (!member.getPw().equals(pw)) {
				...
				manager.update_log(member.getUser_id());
				return member;
			}
		}
	}
	...
	public void logout(HttpSession session, HttpServletResponse response) throws Exception {
		...
		PrintWriter out = response.getWriter();
		out.println("<script>");
		out.println("location.href='./move_index.do';"); // ./move_index.do';
		out.println("</script>"); // document.referrer;
		out.close();
	}
	...
	public void find_pw(HttpServletResponse response, MemberDTO member) throws Exception {
		...
		// 가입에 사용한 이메일이 아니면
		else if (!member.getEmail().equals(manager.login(member.getUser_id()).getEmail())) {
	...
	public MemberDTO update_mypage(MemberDTO member) throws Exception {
		...
		return manager.login(member.getUser_id());
	}
	...
	public MemberDTO update_pw(MemberDTO member, String old_pw, HttpServletResponse response) throws Exception {
		response.setContentType("text/html;charset=utf-8");
		PrintWriter out = response.getWriter();
		if (!old_pw.equals(manager.login(member.getUser_id()).getPw())) {
			out.println("<script>");
	...
	public MemberDTO getUserInfo(String user_id) throws Exception {
		return manager.getUserInfo(user_id);
	}
